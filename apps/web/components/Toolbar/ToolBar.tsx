...
const ToolBar = () => {
  const { activeTool, setSelectedTool } = useToolStore();
      ...
      <div className='fixed bottom-4 left-1/2 -translate-x-1/2 flex items-end gap-2'>
        ...
        <div className='flex bg-white p-2 rounded-xl items-center shadow-2xl border border-neutral-200 gap-2'>
          {iconLibrary.slice(0, 8).map(icon => (
            <button
              key={icon.name}
              className={`rounded-lg p-2 transition-colors ${(activeTool === icon.name) ? 'hover:bg-blue-700 bg-blue-700 text-neutral-50' : 'hover:bg-neutral-100'}`} onClick={() => setSelectedTool(icon.name)}>
              <icon.icon size={20} />
            </button>
          ))}
          ...
          {selectedShape &&
            <button
              className={`rounded-lg p-2 transition-colors ${(activeTool === selectedShape.name) ? 'bg-blue-700 hover:bg-blue-700 text-white' : 'hover:bg-neutral-100 text-neutral-700'}`} onClick={() => setSelectedTool(selectedShape.name)}>
